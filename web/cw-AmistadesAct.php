...
$ccc=mysql_num_rows(db_query("SELECT c.id FROM ({$db_prefix}amistad AS c) WHERE c.amigo='{$ID_MEMBER}' AND c.acepto=0 LIMIT 1", __FILE__, __LINE__));
if($ccc){
...
while($red=mysql_fetch_array($redddbnn)){
...
while($redc=mysql_fetch_array($reddd3)){
