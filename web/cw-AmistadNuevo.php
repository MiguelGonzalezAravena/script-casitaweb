...
while($data=mysql_fetch_assoc($datosmem)){$ser=$data['ID_MEMBER'];$name=$data['realName'];}
...
$errorrd=mysql_num_rows(db_query("SELECT user,amigo FROM {$db_prefix}amistad WHERE (user='$yo' AND amigo='$ser' OR user='$ser' AND amigo='$yo') AND acepto=0 LIMIT 1",__FILE__, __LINE__));
