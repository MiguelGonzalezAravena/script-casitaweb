...
$da=db_query("SELECT id_user FROM ({$db_prefix}muro) WHERE id='{$_POST['id']}' AND tipocc='0' ORDER BY id DESC LIMIT 1", __FILE__, __LINE__);while($vdd=mysqli_fetch_assoc($da)){$DeQuien=$vdd['id_user'];}mysqli_free_result($da); 
...
while($vdds=mysqli_fetch_assoc($da2)){$ts[]=$vdds['de'];$ts[]=$DeQuien;}mysqli_free_result($da2); 
