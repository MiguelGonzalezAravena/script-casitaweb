...
if($_GET['can'] < 1){$can='20';}elseif($_GET['can']<=5){$can='5';}
elseif($_GET['can'] > 49){$can='50';}elseif($_GET['can'] < '50'){$can=$_GET['can'];}
if(!$_GET['cat']){$cat='';}elseif($_GET['cat']==='142'){$cat='';}else{$cat="m.ID_BOARD={$_GET['cat']} AND ";}
if(!$_GET['an']){$an='183';}else{$an=$_GET['an']-17;}
$rs=db_query("
...
<div class="item"><a target="_blank" title="<?php echo $post['titulo'];?>" href="http://casitaweb.net/post/<?php echo $post['ID_TOPIC'];?>/<?php echo $post['description'];?>/<?php echo urls(censorText($post['titulo']));?>.html" class="categoriaPost <?php echo $post['description'];?>"><?php echo censorText($post['titulo']);?></a></div>

<?php } ?>
<center><a href="http://casitaweb.net/" target="_parent">[ Ver m&aacute;s posts ]</a></center></body></html>