...
if (!$name) {
  ...
  if ($name == 'rigo') {
    ...
    if ($name == $user_settings['realName']) {
      echo'<div class="noesta" style="width:754px;">No puedes banearte a vos mismo.</div>';
    } else {
      if (!empty($estar)) {
        if ($mod <> $ID_MEMBER) {
      ...
      if (!empty($haygentea)) {
        ...
        if (empty($_POST['agregar']) || empty($_POST['modificar'])) {
          echo 'Este usuario esta en la lista de baneados - <a href="' . $boardurl . '/moderacion/edit-user/ban/buscar&usuario=' . $name . '&si=bloke"><b>Buscarlo</b></a>.-';
        } else {
      ...
      echo '
                  ...
                  <tr>
                    <td colspan="2" align="center">
                      <input class="login" onclick="return chekkk(' . $claveeeennJS3 . 'this.form.reason.value);" type="submit" name="' . ($estar ? 'modificar' : 'agregar') . '" value="' . ($estar ? 'Editar ban' : 'Agregar ban') . '" />
                    </td>
                  </tr>
                </table>
              </td>
