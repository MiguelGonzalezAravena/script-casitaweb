...
if($cant){db_query("UPDATE {$db_prefix}members SET notificacionMonitor='0' WHERE ID_MEMBER='{$user_settings['ID_MEMBER']}' LIMIT 1", __FILE__, __LINE__);}
...
if(empty($data['leido'])){$backCOLOR='background-color: #FDFBE7;';}else{$backCOLOR='';}
...
<div style="width:728px;"><strong style="font-size:14px;"><a href="/perfil/'.$realName.'">'.$realName.'</a></strong><br />'.notificacionQUE($data['que'],$data['url'],$data['extra']).'<br />'.hace($data['fecha']).'</div></div><div class="clearfix"></div>';
...
<div style="width:120px;float:right;" align="center"><?php anuncio_160x600(); ?></div>
