...
<script type="text/javascript">function enviarFORMbaneados(){
if($("#usuario").val()==''){$("#usuario").focus(); return false;}
location.href='/moderacion/edit-user/ban/buscar/&usuario='+$("#usuario").val()+'&si=Buscar';
return;}
</script>
...
echo'<div style="padding:8px;background-color:#f4f4f4;border:1px solid #ccc;"><div style="float:right;"><div style="float:left;"><input title="Busca usuario" onfocus="foco(this);" onblur="no_foco(this);" id="usuario" size="30" type="text" value="'.$usuario.'" /></div><div style="float:left;"><input onclick="enviarFORMbaneados();" class="pointer" type="button" name="si" value="Buscar"/></div></div><div class="clearfix"></div></div>';

echo'<div style="margin-top:5px;width:922px;">';
...
if(isset($_GET['pag-seg-15487135'])){$RegistrosAEmpezar=($_GET['pag-seg-15487135']-1)*$RegistrosAMostrar;
$PagAct=$_GET['pag-seg-15487135'];}else{$RegistrosAEmpezar=0;$PagAct=1;}

$NroRegistros=mysql_num_rows(db_query("SELECT a.ban_time FROM ({$db_prefix}ban_groups AS a)", __FILE__, __LINE__));
 $PagAnt=$PagAct-1;
 $PagSig=$PagAct+1;
...
while($ban=mysql_fetch_array($bdsee)){
$ban['ban_time']=hace($ban['ban_time']);
$ban['expires']=$ban['expire_time'] === null ? $txt['never'] : ($ban['expire_time'] < time() ? '<span style="color: red">'.$txt['ban_expired'].'</span>' : (int)ceil(($ban['expire_time'] - time()) / (60 * 60 * 24)) . '&nbsp;d&iacute;a(s)');
