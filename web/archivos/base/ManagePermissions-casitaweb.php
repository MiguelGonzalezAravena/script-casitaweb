...
echo'<form action="' . $scripturl . '?' . $urlSep . '=permissions;sa=quick" method="post" accept-charset="', $context['character_set'], '" name="permissionForm" id="permissionForm">
	...
	foreach ($context['groups'] as $group)
	{
		echo '
				<tr>
					<td class="windowbg2">', $group['name'], $group['id'] == -1 ? ' ' : ($group['id'] == 0 ? '' : ($group['id'] == 1 ? ' ' : ($group['id'] == 3 ? ' ' : ''))), '</td>
...
function template_modify_group() {
	...
	echo '
					...
					<td colspan="2" align="right"><input class="login" type="submit" value="', $txt['permissions_commit'], '" />&nbsp;</td>
				</tr>
			</table>
			<input type="hidden" name="sc" value="', $context['session_id'], '" />
		</form>';
...
';
	...
	echo '
			<tr class="windowbg2">
				<td width="50%" align="right"><label for="permission_enable_deny_check">', $txt['permission_settings_enable_deny'], '</label> :</td>
				<td>
				...
				<td width="50%" align="right"><label for="permission_enable_postgroups_check">', $txt['permission_settings_enable_postgroups'], '</label> :</td>
				<td>
				...
				<td width="50%" align="right"><label for="permission_enable_by_board_check">', $txt['permission_settings_enable_by_board'], '</label> ;">?</a>):</td>
				<td>
