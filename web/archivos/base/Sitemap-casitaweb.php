...
<div class="box_rss"><img src="' . $tranfer1 . '/blank.gif" style="width: 16px; height: 16px;" border="0" alt="" /></div></div><div class="windowbg" style="padding: 4px; width: 292px;"><span class="size11">';
  foreach ($context['boards'] as $board) {
    echo '<a href="' . $boardurl . '/categoria/' . $board['description'] . '" title="' . $board['name'] . '">' . $board['name'] . '</a><br />';
  }
  echo '</span></div></div>
...
<a href="' . $boardurl . '/rss/ultimos-comment/" title="&Uacute;ltimos comentarios">&Uacute;ltimos comentarios</a>
  ...
  foreach ($context['sitemap']['board'] as $board) {
    ...
    echo '<a title="', $board['name'], '" href="' . $boardurl . '/categoria/', $board['id'], '" title="', $board['name'], '"><span title="', $board['name'], '">', $board['name'], '</span></a>';
  }
  echo '</div>';
}
