...
function template_main() {
    ...
    if ($context['allow_admin']) {
        echo '<script type="text/javascript" src="' . $tranfer1 . '/js/?a=3"></script>
...
<br /><b class="size11">Categor&iacute;a:</b><br />';
    ...
    if ($context['allow_admin']) {
        echo '<br/><b class="size11">Color:</b><br/><input type="text" value="" onfocus="foco(this);" onblur="no_foco(this);" name="colorsticky" size="15" onclick="startColorPicker(this)"id="colorsticky"  onkeyup="maskedHex(this)" /><div id="coloraca" style="display:none;margin:0px;padding:0px;"></div>';
    }
    echo '<br /><font class="size11"><b>Opciones:</b></font>';

    if ($context['allow_admin']) {
        echo '<br /><label for="principal"><input type="checkbox" name="principal" id="principal" value="1" /> Agregar a Sticky</label>';
    }

    if ($context['user']['is_admin']) {
        echo '<br /><label for="anuncio"><input type="checkbox" name="anuncio" id="anuncio" value="1" /> Mostrar post como Anuncio</label>';
    }

    if ($user_settings['posts'] >= '200') {
        echo '<br /><label for="nocom"><input type="checkbox" name="nocom" id="nocom" value="1" /> No permitir comentarios</label>';
    }

    echo '<br /><label for="privado"><input type="checkbox" name="privado" id="privado" value="1" /> Solo usuarios registrados</label>';

    echo '<center><input onclick="vprevia(this.form.titulo.value,this.form.contenido.value,this.form.tags.value,this.form);" class="button" style="font-size: 15px;" value="Previsualizar" title="Previsualizar" type="button" tabindex="8" /></center>';
    echo '<br />';
