...
function template_main() {
  global $ID_MEMBER, $context, $modSettings, $settings, $options, $txt, $db_prefix, $scripturl, $sourcedir, $tranfer1, $boardurl;
  ...
  if ($busqueda || $usuario) {
    ...
    if ($cat < 1) {
      ...
      $cats = ' AND p.ID_BOARD=' . $cat;
    }

    if (!$busqueda) {
      $ssdeeesss2 = '';
      $score = "p.posterName LIKE '%$usuario%'";
      if ($usuario) {
        $ssdeeesss = "p.posterName = '$usuario'";
      } else {
        $ssdeeesss = '';
      }
    } else {
      $ssdeeesss2 = "MATCH (p.subject, p.body) AGAINST ('$busqueda')";
      $score = "MATCH (p.subject, p.body) AGAINST ('$busqueda')";
      if ($usuario) {
        $ssdeeesss = "AND p.posterName = '$usuario'";
      } else {
        $ssdeeesss = '';
      }
    }
    ...
    $result = db_query("
      SELECT $score AS Score, p.ID_TOPIC, p.puntos, p.subject, p.posterTime, c.description, p.hiddenOption
      FROM {$db_prefix}messages AS p, {$db_prefix}boards AS c, {$db_prefix}members AS u
      WHERE $ssdeeesss2 $cats $ssdeeesss
      AND p.ID_BOARD = c.ID_BOARD
      $shas
      AND p.ID_MEMBER = u.ID_MEMBER
      AND p.eliminado = 0
      ORDER BY $dbor
