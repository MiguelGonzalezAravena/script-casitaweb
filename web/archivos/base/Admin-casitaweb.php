...
require($sourcedir."/IpModLog.php");
$dat=explode(' : ',$glee[$context['user']['name']]);
$dataMOstrar='<div class="noesta-am" style="width: 922px;margin-bottom:8px;">&Uacute;ltima IP registrada en la moderacion fue '.$dat[0].' y hace '.hace($dat[1]).' que se registr&oacute;. Tu ip Actual es: '.$_SERVER["REMOTE_ADDR"].'</div>';

$dataMOstrar2='<div class="noesta-am" style="width: 922px;margin-bottom:8px;">Actualizando IP...</div>';

if($glee[$context['user']['name']]){
if(!$dat[1]){logipADM(); echo $dataMOstrar2;}

elseif(!$dat[0]){logipADM(); echo $dataMOstrar2;}

elseif(($dat[1]+3600) > time()){echo $dataMOstrar;}

else{logipADM(); echo $dataMOstrar2;}}else{logipADM(); echo $dataMOstrar2;}
...
<option'; if($modSettings['radio']==='1'){echo' selected="selected"';} echo' value="1">CasitaWeb!</option>
...
<tr valign="top"><td valign="top" style="border:1px solid #7C7C7C;">Suma dudosa de Puntos.</td><td valign="top" style="border:1px solid #7C7C7C;">* 30 d&iacute;as.<br/>* Definitivo, en caso de haber sido suspendido y recurre en igual falta.</td><td valign="top" style="border:1px solid #7C7C7C;">Si es necesario.-</td></tr>
<tr valign="top"><td valign="top" style="border:1px solid #7C7C7C;">IP Clonada.</td><td style="border:1px solid #7C7C7C;">* 10 d&iacute;as.<br/>* Definitivo, en caso de haber sido suspendido y recurre en igual falta.</td><td style="border:1px solid #7C7C7C;">No es necesario.-</td></tr></table>

* Si un usuario es suspendido por un <b>MOD</b> &eacute;se usuario <b>DEBE</b> cumplir la pena.<br/>
* Si un <b>MOD</b> levanta la suspensi&oacute;n de un Usuario sin consulta previa al que lo realiz&oacute;, se lo deshabilitar&aacute; de <b>CasitaWeb!</b> tarde o temprano.<br/>
* <b>JAMAS</b> deber&aacute;n bannearse entre <b>MOD&#8217;s</b>, el que lo haga deber&aacute; atenerse a la decisi&oacute;n que tome el resto de sus compa&ntilde;eros.
<div class="hrs"></div><b class="size12">Puntos importante:</b><br /><span class="size11" style="color:green;">* Cuando se elimina un post y la causa es RePost agregar el ID del post principal (No es necesario agrear link completo, con ID solo ya esta).<br />* Cuando en un post est&aacute; roto el enlace fijarse si se puede repara, si no se puede eliminar post.<br />* Solo los posts en la categor&iacute;a Noticias requieren fuente.</span><br/>
<div class="hrs"></div><b class="size12">Protocolo para moderadores:</b><br /><span class="size11">* Un moderador es un usuario com&uacute;n con mayores privilegios los cuales implican mayor responsabilidad.<br />
...
if($context['can_change_permissions']){echo'<tr class="windowbg"><th width="50%" align="right" valign="top"><label for="search_posts_groups">Quien puede buscar:</label></th><td>';theme_inline_permissions('search_posts');echo '</td></tr>';}

echo'<tr class="windowbg"><th align="right"><label for="search_results_per_page_input">Cantidad de resultados por pagina:</label></th><td><input type="text" onfocus="foco(this);" onblur="no_foco(this);" name="search_results_per_page" id="search_results_per_page_input" value="'.$modSettings['search_results_per_page'].'" style="width:20px;" /></td></tr>

	<tr class="windowbg"><th align="right"><label for="timeLoadPageEnable">Mostrar tiempo de creado de p&aacute;gina:</label></th><td><input name="timeLoadPageEnable" id="timeLoadPageEnable" '; if($modSettings['timeLoadPageEnable']=='1'){ echo'checked="checked" ';} echo' value="1" type="checkbox" /></td></tr>
	
	<tr class="windowbg"><th align="right"><label for="smiley_enable">Habilitar emoticones:</label></th><td><input name="smiley_enable" id="smiley_enable" '; if($modSettings['smiley_enable']=='1'){ echo'checked="checked" ';} echo' value="1" type="checkbox" /></td></tr>
	
	<tr class="windowbg"><th align="right"><label for="enableStickyTopics">Habilitar sticky:</label></th><td><input name="enableStickyTopics" id="enableStickyTopics" '; if($modSettings['enableStickyTopics']=='1'){ echo'checked="checked" ';} echo' value="1" type="checkbox" /></td></tr>

	<tr class="windowbg"><th align="right"><label for="puntos_por_post-img">Puntos a dar por post/imagen:</label></th><td><input name="puntos_por_post-img" id="puntos_por_post-img" value="'.$modSettings['puntos_por_post-img'].'" type="text" style="width:20px;" /></td></tr>

 	<tr class="windowbg"><th align="right"><label for="titlog">Cantidad de "&Uacute;ltimos comentarios":</label><br/><spam class="size9">En el inicio</span></th><td><input type="text" onfocus="foco(this);" onblur="no_foco(this);" name="ccomentarios" id="ccomentarios" value="'.$modSettings['catcoment'].'" style="width:20px;" /></td></tr>

    <tr class="windowbg"><td align="right" colspan="2"><input type="submit" name="save" class="login" value="Guardar cambios" /></td></tr></table><input type="hidden" name="sc" value="', $context['session_id'], '" />
...
<table align="center">
	...
	foreach ($context['censored_words'] as $vulgar => $proper)
		echo '
									<div style="margin-top: 1ex;"><input type="text" onfocus="foco(this);" onblur="no_foco(this);" name="censor_vulgar[]" value="', $vulgar, '" size="20" /> => <input type="text" onfocus="foco(this);" onblur="no_foco(this);" name="censor_proper[]" value="', $proper, '" size="20" /></div>';
	echo '		<noscript>
										<div style="margin-top: 1ex;"><input type="text" onfocus="foco(this);" onblur="no_foco(this);" name="censor_vulgar[]" size="20" /> => <input type="text" onfocus="foco(this);" onblur="no_foco(this);" name="censor_proper[]" size="20" /></div>
									</noscript>
                                        ...
                                        function setOuterHTML(element, toValue)
{
	...
	else
	{
		...
		element.parentNode.replaceChild(range.createContextualFragment(toValue), element);
								...
								<td colspan="2"><div class="hrs"></div></td>
							</tr><tr>
	...
	echo '
			...
			<input type="hidden" name="sc" value="', $context['session_id'], '" /></div></div>
		</form>';
}
...
function logipADM(){global $context,$sourcedir;
