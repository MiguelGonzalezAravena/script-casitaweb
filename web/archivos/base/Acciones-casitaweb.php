...
if($asfff==50)echo'<br />';
if($asfff==100)echo'<br />';
if($asfff==150)echo'<br />';
if($asfff==200)echo'<br />';
}
echo'</center></div></div>';}
...
if($tag['hiddenOption']=='1' && $context['user']['is_guest'])echo'<div class="icon_img" style="float:left;margin-right:0px;"><img alt="" src="'.$tranfer1.'/icons/cwbig-v1-iconos.gif?v3.2.3" style="margin-top:-578px;display:inline;" /></div>';
...
if($PagAct>1)echo "<a href='/tags/buscar/&q=$pasda&orden=$order&categoria=$cat2&pag=$PagAnt'>&#171; anterior</a>";
 if($PagAct<$PagUlt)  echo "<a href='/tags/buscar/&q=$pasda&orden=$order&categoria=$cat2&pag=$PagSig'>siguiente &#187;</a>";
...
if(empty($_GET['categoria'])){$cat='p.ID_BOARD';}else{$cat=(int)$_GET['categoria'];}
if(empty($_GET['categoria'])){$cat2='0';}else{$cat2=$cat;}

if($_GET['orden']=='fecha'){$order2=' p.ID_TOPIC DESC';}elseif($_GET['orden']=='puntos'){$order2=' p.puntos DESC';}else{$order2=' p.ID_TOPIC DESC';}
...
if($PagAct > 1) { echo"<a href='/tags/buscar/&q=$pssads&orden=$order&categoria=$cat2&pag=$PagAnt'>&#171; anterior</a>";}

if($PagAct<$PagUlt){ echo "<a href='/tags/buscar/&q=$pssads&orden=$order&categoria=$cat2&pag=$PagSig'>siguiente &#187;</a>";}
echo'<div class="clearBoth"></div></div>';}
...
$id=(int)$_GET['id'];
if(empty($id)){fatal_error('Debes seleccionar la im&aacute;gen.-', false);}
...
function template_tyc6(){global $tranfer1, $context;
...
global $tranfer1, $context, $settings, $options, $txt, $scripturl,$db_prefix, $modSettings;
...
$rs=db_query("SELECT b.name,b.ID_BOARD FROM ({$db_prefix}boards AS b) WHERE b.ID_BOARD<>142", __FILE__, __LINE__);
echo'<div><div class="box_buscador">
<div class="box_title" style="width: 919px;"><div class="box_txt box_buscadort"><center>WidGet</center></div>
<div class="box_rss"><img src="'.$tranfer1.'/blank.gif" style="width: 14px; height: 12px;" alt="" border="0" /></div></div>
<div style="width:911px;padding:4px;margin-bottom:8px;" class="windowbg">
      Integra los &uacute;ltimos posts de CasitaWeb! en tu Web y estate siempre actualizado.<br />

      En solo segundos podr&aacute;s tener un listado que estar&aacute; siempre 
      actualizado con los &uacute;ltimos posts publicados en CasitaWeb!.<br />
...
<td align="center" style="padding:0px;margin:0px;" class="windowbg">
     <textarea onfocus="foco(this);" onblur="no_foco(this);" id="codigo" cols="47" rows="6" onClick="focus_code();"></textarea></td>
...
$_GET['pag']=isset($_GET['pag']) ? $_GET['pag'] : '';
if($_GET['pag'] < 1){$dda=1;}else{$dda=$_GET['pag'];}
if(isset($dda)){
...
echo'</div>';}
 ...
 $PagSig=$PagAct+1;
 $PagUlt=$NroRegistros/$RegistrosAMostrar;
 $Res=$NroRegistros%$RegistrosAMostrar;}else{echo'<div class="noesta">No hay denuncias.</div>';}
...
echo'<div class="windowbgpag" style="width:378px;">';
 if($PagAct>1) echo "<a href='/moderacion/denuncias/pag-$PagAnt'>&#171; anterior</a>";
 if($PagAct<$PagUlt)  echo "<a href='/moderacion/denuncias/pag-$PagSig'>siguiente &#187;</a>";
echo'</div>';}
}
...
function template_quickreply_box(){global $tranfer1, $db_prefix,$context,$txt;
