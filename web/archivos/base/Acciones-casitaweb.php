...
function template_tyc24() {
  global $context, $user_settings, $modSettings, $tranfer1, $mbname, $boardurl;
...
function template_tyc666() {
  ...
  $ddcc = isset($_GET['q']) ? seguridad($_GET['q']) : '';
  ...
  if ($ddcc) {
    ...
    $request = db_query("
      SELECT p.puntos, p.subject, p.ID_TOPIC, b.description, p.hiddenOption, p.posterTime
      FROM {$db_prefix}tags AS t, {$db_prefix}messages AS p,{$db_prefix}boards AS b
      WHERE t.palabra LIKE '%$pssads%'
      AND t.id_post = p.ID_TOPIC
      $shas
    ...
    if (!$NroRegistros) {
      ...
      echo '
        <table class="linksList" style="width: 922px;">
          <thead align="center">
            <tr>
              <th style="text-align: left;">' . $NroRegistros . ' Posts con el tag: ' . $pssads . '</th>
              <th>Fecha</th>
              <th>Puntos</th>
            </tr>
          </thead>
...
function template_tyc12() {
  ...
  if (empty($id)) {
    fatal_error('Debes seleccionar la imagen.', false);
  }
  ...
  echo '
    <div>
      <div class="box_buscador">
        ...
        <div style="width: 912px; padding: 4px;" class="windowbg">
          <center>
            <form action="' . $boardurl . '/web/cw-EnviarImgMail.php" method="post" accept-charset="' . $context['character_set'] . '">
              <br />
              <font class="size11">
                <b>Recomendarle esta imagen hasta a seis amigos:</b>
              </font>
              <br />
              <b class="size11">1 - </b>
...
function template_denuncias() {
  ...
  echo '
    ...
    <div style="width: 922px;">
      <div class="box_title">
        <div class="box_txt box_buscadort">
          <center>Denuncias de posts, im&aacute;genes, usuarios y comunidades</center>
        </div>
      </div>
    </div>
    <div style="width: 912px; padding: 4px;" class="windowbg">';
  ...
  if ($NroRegistros) {
    ...
    foreach ($context['denunciasss'] as $den1) {
      ...
      echo '
        <div style="display: none; margin: 8px; width: 896px;" id="resultado_' . $den1['id_post'] . '"></div>
        <img style="display: none; margin-left: 8px; margin-top: 8px;" id="cargando_' . $den1['id_post'] . '" src="' . $tranfer1 . '/icons/cargando.gif" width="16px" height="16px" alt="" />
        <div ' . $estilo . ' id="contentv_' . $den1['id_post'] . '">
        ...
        echo '
          <div>
            <div style="float: left; margin-right: 5px; margin-left: 5px;">
              <a onclick="actuarDenuncia(\'Acep\', \'' . $den1['id_post'] . "', '" . $den1['id_user'] . "', '" . $den1['id_denuncia'] . '\');" class="botN1" style="cursor: pointer; color: #fff; text-shadow: #005400 0px 1px 0px;">Aceptar</a>
            </div>
            <div>
              <a onclick="actuarDenuncia(\'Rech\', \'' . $den1['id_post'] . "', '" . $den1['id_user'] . "', '" . $den1['id_denuncia'] . '\');" class="botN2" style="cursor: pointer; text-shadow: #CC0000 0px 1px 0px; color: #fff;">Rechazar</a>
            </div>
          </div>';
      } else {
        echo '
          <div>
            -
            Atendido por
            <a title="' . $den1['mod'] . '" href="' . $boardurl . '/perfil/' . $den1['mod'] . '">' . $den1['mod'] . '</a>
          </div>';
      }
      ...
      while ($den2 = mysqli_fetch_assoc($reques2)) {
        ...
        echo '
          <span class="size11">
            <b class="size11">Denunciante:</b>
            <a href="' . $boardurl . '/perfil/' . $den2['realName'] . '" title="' . $den2['realName'] . '">' . $den2['realName'] . '</a>
            |
            <b class="size11">IP:</b>
            <a href="http://lacnic.net/cgi-bin/lacnic/whois?query=' . $den2['memberIP'] . '" title="' . $den2['memberIP'] . '">' . $den2['memberIP'] . '</a>
            |
  ...
  echo '
        <div class="hrs"></div>
        <b style="color: #D5D5D5;" class="size13">Esperando moderaci&oacute;n</b>
        |
        <b style="color: #FE2E2E;" class="size13">Denuncia rechazada</b>
        |
        <b style="color: #47973A;" class="size13">Denuncia aceptada</b>
        <br />
        <span class="size11">
          <b style="color: #008000;">Aceptar:</b>
          El denunciante denunci&oacute;
          <span style="color:#008000;">bien</span>, se le incrementar&aacute; 1 punto al denunciante
          -
          <b>ACTUAR SOBRE LA DENUNCIA</b>
          <br />
          <b style="color: #FE0000;">Rechazar:</b>
          El denunciante denunci&oacute;
          <span style="color: #FE0000;">mal</span>.
        </span>
      </span>
    </div>';
