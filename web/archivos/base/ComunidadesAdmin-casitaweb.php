      ...
      while ($row = mysqli_fetch_assoc($datos)) {
        ...
        echo '
          <tr id="ID_' . $row['id'] . '">
            ...
            <td>';
        if (!$row['bloquear_razon']) {
          echo ' - ';
        } else {
          echo $row['bloquear_razon'];
        }
        echo '</td>
<td>';
        if (!$row['bloquear_por']) {
          ...
          echo '<a href="/perfil/' . $row['bloquear_por'] . '">' . $row['bloquear_por'] . '</a>';
        }
        echo '</td>
</tr>';
      }
    ...
    if ($PagAct > 1 || $PagAct < $PagUlt) {
      ...
      if ($PagAct > 1)
        echo "<a href='/moderacion/comunidades/pag-$PagAnt'>&#171; anterior</a>";
      if ($PagAct < $PagUlt)
        echo "<a href='/moderacion/comunidades/pag-$PagSig'>siguiente &#187;</a>";
...
<td><a href="/perfil/' . $row['UserName'] . '">' . $row['UserName'] . '</a></td>
    ...
    if ($PagAct > 1 || $PagAct < $PagUlt) {
      echo '<div class="windowbgpag" style="width:300px;">';
      if ($PagAct > 1)
        echo "<a href='/moderacion/comunidades/temas/pag-$PagAnt'>&#171; anterior</a>";
      if ($PagAct < $PagUlt)
        echo "<a href='/moderacion/comunidades/temas/pag-$PagSig'>siguiente &#187;</a>";
      echo '</div><div class="clearBoth"></div>';
    }
  }
}
