...
<div class="post-contenido" property="dc:content">';
  ...
  if ($context['user']['is_guest']) {
    echo '<div align="center" style="-moz-border-radius: 5px;-webkit-border-radius:5px;display:block;margin-bottom:10px;margin-top:25px;padding:2px;border: solid 1px #D5CCC3;background:#FFF;"><a href="/registrarse/" style="font-size:12px;color:#FFB600;margin-bottom:3px;"><b>REGISTRATE GRATIS Y ELIMINA ESTA PUBLICIDAD, ADEMAS TENDRAS ACCESO A TODOS LOS POSTS Y FUNCIONES</b></a><br />';
    anuncio_728x90();
    echo '</div>';
  }
    ...
    if ($context['user']['is_admin']) {
      ...
      if ($context['id_cat'] !== '142') {
        if ($context['user_ID'] <> $context['user']['id']) {
          echo ' return errorrojo2(this.form.causa.value);';
        }
      }
      echo '" type="submit" />';
      if ($context['id_cat'] !== '142') {
        if ($context['user_ID'] <> $context['user']['id']) {
          echo '&nbsp;<b>Causa:</b>&nbsp;<input type="text" onfocus="foco(this);" onblur="no_foco(this);" id="causa" name="causa" maxlength="70" style="width:135px;" /><center><label id="errors" class="size10" style="color: red;"></label></center>';
        }
      }
      echo '</form>';
    }
...
<form action="/web/cw-postEliminar.php?id=' . $context['id-post'] . '" method="post" accept-charset="' . $context['character_set'] . '" style="margin:0px;padding:0px;" name="causa" id="causa">';
    ...
    if (!empty($context['leecher'])) {
      echo '<b class="size11"><center>Usuarios no registrados y <span title="Primer rango">turistas</span> no puede calificar.</center></b><div class="hrs"></div>';
    } else {
      if ($context['id_cat'] == 45 || $context['id_cat'] == 132) {
        echo '<center><b class="size11">No se permite puntuar los post de esta categor&iacute;a.</b></center><div class="hrs"></div>';
      } else {
...
ORDER BY p.id DESC", __FILE__, __LINE__);
      ...
      if (!empty($skasdasdbsddd)) {
        echo '<div class="hrs"></div><b>Dieron puntos a este post:</b> ';
        echo join(', ', $userdasd);
      }
    }
    echo '<div class="hrs"></div>
...
<div style="float:left;">';
      ...
      echo '
        <b id="autor_cmnt_' . $coment['id'] . '" user_comment="' . $coment['nomuser'] . '" text_comment=\'' . $coment['comentario2'] . '\'><a href="' . $boardurl . '/perfil/' . $coment['nommem'] . '" style="color: #956100;">' . $coment['nomuser'] . '</a></b>
  ...
  if ($context['is_locked'] && $context['user']['id']) {
    echo '<div id="post_cerrado" class="noesta-am" style="width: 774px;margin-top:5px;">Este post esta cerrado, por lo tanto no se permiten nuevos comentarios.</div>';
  }
...
<!-- fin comentar -->';
  ...
  if (!$context['user']['id']) {
    echo '<p align="right" style="font-text:9px;"><a href="' . $boardurl . '/noestilo/post/' . $context['id-post'] . '">Sin estilo</a></p>';
  }
}
