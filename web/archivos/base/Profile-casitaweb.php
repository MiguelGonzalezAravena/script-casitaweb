...
if($lugar=='muro'){$muro=' class="activado" ';$amist='';$Comu='';$apaa='';}
elseif($lugar=='apariencia'){$muro='';$amist='';$Comu='';$apaa=' class="activado" ';}
elseif($lugar=='comunidades'){$muro='';$amist='';$Comu=' class="activado" ';$apaa='';}
...
elseif($lugar==='2' || $lugar=='muro'){$tipo='0';}
elseif($lugar==='5' || $lugar=='comunidades'){$tipo='5';}
else{$tipo=$lugar;}

$noimg = $context['member']['name'] . '&nbsp;no&nbsp;tiene&nbsp;ning&uacute;na&nbsp;imagen';
...
echo'<script type="text/javascript">function ActivarUS(id){
	$.ajax({
		type: \'GET\',
		url: \'/web/cw-ActivarUser.php\',
        data: \'u=\' + id,
...
if(!empty($cantidaddss)==='1'){echo'<p style="font-size:10px;margin:0px;padding:0px;float:left;width:250px;"><a href="/perfil/'.$context['member']['name'].'/amigos-en-comun/">1 amigo en com&uacute;n</a></p>';}
elseif(!empty($cantidaddss) >= '2'){echo'<p style="font-size:10px;margin:0px;padding:0px;float:left;width:250px;"><a href="/perfil/'.$context['member']['name'].'/amigos-en-comun/">'.$cantidaddss.' amigos en com&uacute;n</a></p>';}

if(!empty($bbvxc)==='1'){echo'<p align="right" style="font-size:10px;margin:0px;padding:0px;width:250px;"><a href="/perfil/'.$context['member']['name'].'/lista-de-amigos/">1 amigo</a></p>';}
elseif(!empty($bbvxc) >= '2'){echo'<p align="right" style="font-size:10px;margin:0px;padding:0px;"><a href="/perfil/'.$context['member']['name'].'/lista-de-amigos/">'.$bbvxc.' amigos</a></p>';}

if((!empty($cantidaddss)==='1')||(!empty($cantidaddss) >= '2')||(!empty($bbvxc)==='1')||(!empty($bbvxc) >= '2')){
echo'<div class="hrs"></div>';}

$RegistrosAMostrar=15;
if($pagq1 < 1){$dud=1;}else{$dud=$pagq1;}
...
echo'<div class="windowbgpag" style="width:200px;">';
 if($PagAct>1) echo "<a href='/perfil/{$context['member']['name']}/amigos-en-comun-pag-$PagAnt'>&#171; anterior</a>";
 if($PagAct<$PagUlt)  echo "<a href='/perfil/{$context['member']['name']}/amigos-en-comun-pag-$PagSig'>siguiente &#187;</a>";
echo'</div><div class="clearBoth"></div><div style="clear: both;"></div>';}}else{echo'<div class="windowbg" style="border:1px solid #B3A496;width:523px;padding:8px;font-size:11px;"><b class="size11">Acci&oacute;n no reconocida.-</b><hr/></div>';}
...
if(!empty($cantidaddss)==='1'){echo'<p style="margin:0px;padding:0px;float:left;width:250px;"><a href="/perfil/'.$context['member']['name'].'/amigos-en-comun/">1 amigo en com&uacute;n</a></p>';}
elseif(!empty($cantidaddss) >= '2'){echo'<p style="margin:0px;padding:0px;float:left;width:250px;"><a href="/perfil/'.$context['member']['name'].'/amigos-en-comun/">'.$cantidaddss.' amigos en com&uacute;n</a></p>';}

if(!empty($bbvxc)==='1'){echo'<p align="right" style="margin:0px;padding:0px;width:250px;"><a href="/perfil/'.$context['member']['name'].'/lista-de-amigos/">1 amigo</a></p>';}
elseif(!empty($bbvxc) >= '2'){echo'<p align="right" style="margin:0px;padding:0px;"><a href="/perfil/'.$context['member']['name'].'/lista-de-amigos/">'.$bbvxc.' amigos</a></p>';}
if((!empty($cantidaddss)==='1')||(!empty($cantidaddss) >= '2')||(!empty($bbvxc)==='1')||(!empty($bbvxc) >= '2')){
echo'<div class="hrs"></div>';}
...
if($context['user']['is_guest']){echo'<div style="clear: left;"><div class="noesta-am" style="width:541px;margin-bottom:8px;">Para poder comentar este muro es necesarios estar <a href="/registrarse/" style="color:#FFB600;" title="Registrarse">Registrado</a>.<br />Si ya tenes usuario <a href="javascript:irAconectarse();" style="color:#FFB600;" title="Conectarse">Conectate!</a></div></div>';}

else{if($user_settings['ID_MEMBER']===$context['member']['id']){

echo'<form action="/web/cw-comentarMuro.php" method="post" accept-charset="', $context['character_set'], '" style="margin:0px;padding:0px;"><div class="muroQh">
...
$mostrarmuros=db_query("
SELECT m.id_user,m.muro,m.id,m.fecha,m.de,m.tipo,m.ccos
FROM ({$db_prefix}muro AS m)
...
if($cmntarioss){$displaymaM='style="display:inline;" ';}else{$displaymaM='style="display:none;" ';}
echo $yata.'&#32;-&#32;<span onclick="boxHablar(\''.$ivvd.'\');" style="cursor:pointer;color:#424242;" id="c-'.$ivvd.'">Comentar</span>'; echo'<span '.$displaymaM.'id="vmam_'.$ivvd.'">&#32;-&#32;<a href="/perfil/'.$context['member']['name'].'/muro;ccIDmuro='.$ivvd.'">Ver muro a muro</a></span>'; if($user_settings['ID_MEMBER']==$context['member']['id'] || ($user_info['is_admin'] || $user_info['is_mods'])){echo'&#32;-&#32;<span class="pointer" onclick="Boxy.confirm(\'&iquest;Estas seguro que deseas borrar este mensaje?\', function() { del_coment_muro(\''.$ivvd.'\'); }, {title: \'Eliminar Mensaje\'}); " title="Eliminar Mensaje">eliminar</span>';} echo'</div></td></tr></table>';


if($cmntarioss > 0){
$jsAP= isset($_GET['jsAP']) ? (int)$_GET['jsAP'] : '';
if($jsAP==$ivvd){$limit='';}else{$limit=' LIMIT 2';}
$mostrarcmtarios=db_query("
SELECT m.id_user,m.muro,m.id,m.fecha,m.de
FROM ({$db_prefix}muro AS m)
WHERE m.id_cc='{$ivvd}' AND m.tipocc=1
ORDER BY m.id ASC$limit", __FILE__, __LINE__);
...
if(empty($PagAct)){echo'<a style="color:#D35F2C;" href="/perfil/'.$context['member']['name'].'/muro;jsAP='.$ivvd.'#cto_'.$ivvd.'">'.$leerTXT.'</a>';}elseif(!empty($PagAct)){
echo'<a style="color:#D35F2C;" href="/perfil/'.$context['member']['name'].'/muro-pag-'.$PagAct.';jsAP='.$ivvd.'#cto_'.$ivvd.'">'.$leerTXT.'</a>';}}

echo'</div>'.textarea2($ivvd);
}else{echo textarea2($ivvd);}
...
echo'<div class="windowbgpag" style="width:524px;">';
 if($PagAct>1) echo "<a href='/perfil/{$context['member']['name']}/muro-pag-$PagAnt'>&#171; anterior</a>";
 if($PagAct<$PagUlt)  echo "<a href='/perfil/{$context['member']['name']}/muro-pag-$PagSig'>siguiente &#187;</a>";
echo'<div class="clearBoth"></div></div>';}
}else{
//SOLO 1 comentario////////////////////////////
///////////////////////////////////////////////
...
echo'<div class="windowbg" style="border-top:#C8C8C8 solid 1px;width:523px;padding:8px;font-size:11px;">
<div class="muroBug">';
$datosmem=db_query("
SELECT realName,avatar
FROM ({$db_prefix}members)
...
echo'</td><td valign="top" style="margin:0px;font-size:11px;"><strong><a href="/perfil/'.$nombremem.'" title="'.$nombremem.'" style="font-size:14px;color:#D35F2C;">'.$nombremem.'</a></strong><br />'.VideosMuro($filtrado);
echo'<div style="margin-top:6px;">';
echo $yata.'&#32;-&#32;<span onclick="boxHablar(\''.$ivvd.'\');" style="cursor:pointer;color:#424242;" id="c-'.$ivvd.'">Comentar</span>'; if($user_settings['ID_MEMBER']==$context['member']['id'] || ($user_info['is_admin'] || $user_info['is_mods'])){echo'&#32;-&#32;<a onclick="Boxy.confirm(\'&iquest;Estas seguro que deseas borrar este mensaje?\', function() { del_coment_muro(\''.$ivvd.'\'); }, {title: \'Eliminar Mensaje\'}); " href="#" title="Eliminar Mensaje">eliminar</a>';} echo'</div></td></tr></table>';
...
if($MSN){echo'<li><label>Mensajeria:</label><strong>'.$MSN.'</strong></li>';}
if($context['member']['website']['title']){echo'<li><label>Web:</label><strong>'.censorText($context['member']['website']['title']).'</strong></li>';}
...
if(!empty($mddd['peso'])){echo'<li><label>Pesa:</label><strong>'.$mddd['peso'].' kilos</strong></li>';}
if(!empty($mddd['color_de_pelo'])){echo'<li><label>Su color de pelo:</label><strong>'.$mddd['color_de_pelo'].'</strong></li>';}
if(!empty($mddd['color_de_ojos'])){echo'<li><label>Su color de ojos:</label><strong>'.$mddd['color_de_ojos'].'</strong></li>';}
if(!empty($mddd['complexion'])){echo'<li><label>Su fisico:</label><strong>'.$mddd['complexion'].'</strong></li>';}
if(!empty($mddd['mi_dieta_es'])){echo'<li><label>Su dieta es:</label><strong>'.$mddd['mi_dieta_es'].'</strong></li>';}
if(!empty($mddd['fumo'])){echo'<li><label>Fuma:</label><strong>'.$mddd['fumo'].'</strong></li>';}
if(!empty($mddd['tomo_alcohol'])){echo'<li><label>Toma Alcohol:</label><strong>'.$mddd['tomo_alcohol'].'</strong></li>';}
}

if(!empty($mddd['me_gustaria']) || !empty($mddd['en_el_amor_estoy']) || !empty($mddd['hijos'])){
echo'<li class="sep"><h4>M&aacute;s datos</h4></li>';
if(!empty($mddd['me_gustaria'])){echo'<li><label>Le gustaria:</label><strong>'.$mddd['me_gustaria'].'</strong></li>';}
if(!empty($mddd['en_el_amor_estoy'])){echo'<li><label>En el amor esta:</label><strong>'.$mddd['en_el_amor_estoy'].'</strong></li>';}
if(!empty($mddd['hijos'])){echo'<li><label>Hijos:</label><strong>'.$mddd['hijos'].'</strong></li>';}
}
...
if(!empty($mddd['estudios'])){echo'<li><label>Sus estudios:</label><strong>'.$mddd['estudios'].'</strong></li>';}
if(!empty($mddd['profesion'])){echo'<li><label>Profesi&oacute;n:</label><strong>'.censorText($mddd['profesion']).'</strong></li>';}
if(!empty($mddd['empresa'])){echo'<li><label>Empresa:</label><strong>'.censorText($mddd['empresa']).'</strong></li>';}
if(!empty($mddd['nivel_de_ingresos'])){echo'<b>Su nivel de ingresos:</b> '.$mddd['nivel_de_ingresos'].'</strong></li>';}
if(!empty($mddd['intereses_profesionales'])){echo'<li><label>Intereses profesionales:</label><strong>'.censorText($mddd['intereses_profesionales']).'</strong></li>';}
if(!empty($mddd['habilidades_profesionales'])){echo'<li><label>Habilidades Profesionales:</label><strong>'.censorText($mddd['habilidades_profesionales']).'</strong></li>';}
}
...
if(!empty($mddd['mis_intereses'])){echo'<li><label>Intereses:</label><strong>'.censorText($mddd['mis_intereses']).'</strong></li>';}
if(!empty($mddd['hobbies'])){echo'<li><label>Hobbies:</label><strong>'.censorText($mddd['hobbies']).'</strong></li>';}
if(!empty($mddd['series_de_tv_favorita'])){echo'<li><label>Series de Tv favoritas:</label><strong>'.censorText($mddd['series_de_tv_favorita']).'</strong></li>';}
if(!empty($mddd['musica_favorita'])){echo'<li><label>M&uacute;sica favorita:</label><strong>'.censorText($mddd['musica_favorita']).'</strong></li>';}
if(!empty($mddd['deportes_y_equipos_favoritos'])){echo'<li><label>Deportes y Equipos:</label><strong>'.censorText($mddd['deportes_y_equipos_favoritos']).'</strong></li>';}
if(!empty($mddd['libros_favoritos'])){echo'<li><label>Libros Favoritos:</label><strong>'.censorText($mddd['libros_favoritos']).'</strong></li>';}
if(!empty($mddd['peliculas_favoritas'])){echo'<li><label>Pel&iacute;culas favoritas:</label><strong>'.censorText($mddd['peliculas_favoritas']).'</strong></li>';}
if(!empty($mddd['comida_favorita'])){echo'<li><label>Comida favor&iacute;ta:</label><strong>'.censorText($mddd['comida_favorita']).'</strong></li>';}
if(!empty($mddd['mis_heroes_son'])){echo'<li><label>Sus heroes son:</label><strong>'.censorText($mddd['mis_heroes_son']).'</strong></li>';}
}
...
// A TODOS
if(!$dataquien){apariencia($context['member']['id']);}
// A NADIE
elseif($dataquien=='1' && ($user_settings['ID_MEMBER'] != $context['member']['id'])){echo'<div class="noesta">No puedes ver la apariencia de '.$context['member']['name'].'.</div>';}
// A AMIGOS
elseif($dataquien=='2' && (!empty($esfeee) || $user_settings['ID_MEMBER']==$context['member']['id'])){echo'<div class="noesta">Solo amigos de '.$context['member']['name'].' pueden ver la apariencia.</div>';}
// A REGISTRADOS
elseif($dataquien=='3' && $context['user']['is_guest']){echo'<div class="noestaGR">Solo usuarios registrados pueden esta apariencia.</div>';}
// A TODOS
else{apariencia($context['member']['id']);}
...
if($PagAct<$PagUlt){
echo'<div class="windowbgpag" style="width:200px;">';
if($PagAct>1)echo'<a href="/perfil/'.$context['member']['name'].'/comunidades-pag-'.$PagAnt.'">&#171; anterior</a>';
if($PagAct<$PagUlt)echo '<a href="/perfil/'.$context['member']['name'].'/comunidades-pag-'.$PagSig.'">siguiente &#187</a>'; echo'</div>';}
}else{echo'<div class="noestaGR" style="width:541px;">Este usuario no tiene comunidades creadas.</div>';}

}else{echo'<div class="noestaGR" style="width:541px;">Acci&oacute;n no reconocida.-</div>';}
...
if($context['yadio2']){echo'<div class="noesta-am" style="margin-bottom:5px;">Esperando aceptaci&oacute;n de amistad.</div>';}

echo'<div class="BoxUserMenu" align="center"><div style="height:auto;border-top:#fff solid 1px;border-bottom:none;border-right:#fff solid 1px;border-left:#fff solid 1px;padding:4px;">
<div class="AvatarBox">';
if (!empty($context['member']['avatar']['image'])) echo $context['member']['avatar']['image'];else {echo'<img alt="" src="'.$no_avatar.'" border="0" alt="Sin Avatar" onerror="error_avatar(this)" />';}
if($user_info['is_admin'] || $user_info['is_mods']){$d10='<a href="/web/cw-TEMPquienDioptsPost.php?id='.$context['member']['id'].'" title="30 posts" class="boxy">'; $d2='</a>';}
echo'</div></div>
<div class="AccionMem2">';

if(!$user_info['is_guest']){
if($user_settings['ID_MEMBER'] <> $context['member']['id']){
echo'<div class="AccionMem"><a href="/web/cw-TEMPenviarMP.php?user='.$context['member']['name'].'" title="Enviar MP a '.$context['member']['name'].'" class="boxy" >Enviarle MP</a></div>';

if(!$context['yadio2']){echo'<div class="AccionMem">';
if($context['yadio']){echo'<a href="/web/cw-AmistadBorrar.php?user='.$context['member']['name'].'" title="Quitar amistad">Quitar amistad</a>';}
else{echo'<a href="/web/cw-AmistadNuevo.php?user='.$context['member']['name'].'" title="Agregar amistad">Agregar amistad</a>';}echo'</div>';}

echo'<div class="AccionMem"><a  href="#" onclick="Boxy.load(\'/web/cw-denunciaTEMP.php?t=0;d='.$context['member']['name'].'\', {title : \'Denunciar a '.$context['member']['name'].'\'});" title="Denunciar a '.$context['member']['name'].'">Denunciar usuario</a></div>';

if(!$context['mpno']){$nostyle2=' style="display:none;"';$nostyle='';}else{$nostyle2='';$nostyle=' style="display:none;"';}
echo'<div class="AccionMem" id="des"'.$nostyle.'><a href="#" onclick="ignorar(\''.$context['member']['id'].'\',\'2\'); return false;" title="Desdmitir usuario">Desadmitir usuario</a></div>
...
echo'<div class="AccionMem"><a href="/moderacion/edit-user/perfil/', $context['member']['id'], '" >Moderar usuario</a></div>';}
}}
...
<a href="/buscador/&q=&autor='.$context['member']['name'].'&orden=fecha&categoria=0">'.$context['postuser'].'</a></span></div></div>';
...
<div class="Dd"><div style="float:left;"><h4>Im&aacute;genes:</h4></div><div style"float:right;"><span><a href="/imagenes/'.$context['member']['name'].'">'.$context['count'].'</a></span></div></div>
...
if(strlen($post['subject'])>45){$tit2=substr($post['subject'],0,42)."...";}else{$tit2=$post['subject'];}
echo'<li><a href="/post/'.$post['topic'].'/'.$post['board']['description'].'/'.urls($post['subject']).'.html" target="_self" title="'.$post['subject'].'" class="categoriaPost '.$post['board']['description'].'">'.$tit2.'</a></li>';}
echo'</ul>';
if(!empty($context['posts'])){echo'<div class="clearBoth" style="height:21px;"><div class="windowbgpag" style="float:right;"><a href="/buscador/&q=&autor='.$context['member']['name'].'&orden=fecha&categoria=0">ver m&aacute;s</a></div></div>';}
else echo'<div class="noestaGR">'.$nopost.'</div>';
echo'</div>';
...
echo'<div class="clearBoth" style="height:21px;"><div class="windowbgpag" style="float:right;"><a href="/comunidades/buscar/&q=&autor='.$context['member']['name'].'&orden=fecha&categoria=&buscador_tipo=c">ver m&aacute;s</a></div></div>';}

echo'</div>';


echo'<div style="margin-bottom:10px;">';
echo'<div class="title-w"><h3>&Uacute;ltimas imagenes</h3></div>';
if($context['count']){
...
echo'<td><a href="/imagenes/ver/'.$img['id'].'"><img alt="" title="' . $img['title'] . '" style="width: 85px;height: 70px;" class="avatar-box" src="' . $img['filename'] . '" border="0" /></a></td>';
if($rowlevel < ($maxrowlevel+1))$rowlevel++;

else{echo '</tr>';$rowlevel = 0;}
}
echo'</table>';
echo'<div class="clearBoth" style="height:21px;"><div class="windowbgpag" style="float:right;"><a href="/imagenes/'.$context['member']['name'].'">ver m&aacute;s</a></div></div>';}
else{echo'<div class="noestaGR">'.$noimg.'</div>';}
echo'</div>';
...
echo'<div style="float:left;font-size:10px;">';if($cantidaddss=='1'){echo'<p style="margin:0px;padding:0px;"><a href="#" title="1 amigo en com&uacute;n">1 amigo en com&uacute;n</a></p>';}elseif($cantidaddss >= '2'){echo'<p style="margin:0px;padding:0px;"><a href="/perfil/'.$context['member']['name'].'/lista-de-amigos/" title="'.$cantidaddss.' amigos en com&uacute;n">'.$cantidaddss.' amigos en com&uacute;n</a></p>';}
...
$avatar=$row['avatar'];
echo'<td align="center" style="font-size:11px;font-family:arial;margin:0px;padding:0px;"><a href="/perfil/'.$nombremem.'" title="'.$nombremem.'" style="text-decoration:none;">';if(empty($avatar)){echo'<img class="avatar-box" style="width:40px;height:40px;" alt="" src="'.$no_avatar.'" onerror="error_avatar(this)" title="'.$nombremem.'" />';}else {echo'<img style="width:40px;height:40px;" class="avatar-box" alt="" src="'.$avatar.'" onerror="error_avatar(this)" title="'.$nombremem.'" />';}echo'</td>';
if($dasdasdasds++==8){echo'</tr><tr>';}}}
...
echo'<div style="float:left;font-size:10px;">';if($bbvxc=='1'){echo'<p style="margin:0px;padding:0px;"><a href="#">1 amigo</a></p>';}elseif($bbvxc >= '2'){echo'<p style="margin:0px;padding:0px;"><a href="/perfil/'.$context['member']['name'].'/lista-de-amigos/" title="'.$bbvxc.' amigos">'.$bbvxc.' amigos</a></p>';}
...
if(strlen($nombremem)>9){$cortadoname=substr($nombremem,0,6)."...";}else{$cortadoname=$nombremem;}

echo'<td align="center" style="font-size:11px;font-family:arial;margin:0px;padding:0px;"><a href="/perfil/'.$nombremem.'" title="'.$nombremem.'" style="text-decoration:none;">';if(empty($avatar)){echo'<img style="width:40px;height:40px;" alt="" src="'.$no_avatar.'" class="avatar-box" onerror="error_avatar(this)" />';}else {echo'<img style="width:40px;height:40px;" alt="" class="avatar-box" src="'.$avatar.'" onerror="error_avatar(this)" />';}echo'</td>';
if($pds++==8){echo'</tr><tr>';}}
echo'</tr></table></center></div>';}
...
function template_cuenta(){global $tranfer1, $context, $no_avatar;
...
<form name="per" method="post" onsubmit="return load_new_avatar();" action="/web/cw-avatarEditar.php">';
echo'<div class="box_780"><div class="box_title" style="width: 774px;"><div class="box_txt box_780-34"><center>';if($getid){echo'Editar el avatar';}else{echo'Editar mi avatar';}echo'</center></div>';
...
echo'<div class="box_rss"><img alt="" src="'.$tranfer1.'/blank.gif" style="width:16px;height:16px;" border="0" /></div></div><div class="windowbg" style="width: 766px; padding: 4px;margin-bottom:8px;"><table width="100%" cellpadding="4">
						<tr valign="top">
							...
							echo'<img alt="" src="'.$context['avatar_url'].'" width="120" weight="120" align="left" vspace="4" hspace="4" id="miAvatar" onerror="error_avatar(this)" />';		else
							echo'<img alt="" src="'.$no_avatar.'" width="120" weight="120" align="left" vspace="4" hspace="4" id="miAvatar" onerror="error_avatar(this)" />';							
							echo'</div></td>
							<td width="640px" valign="top"><br /><br /><center>Escribe la direcci&oacute;n';if($getid){echo' del ';}else{echo' de tu ';} echo'<i>avatar</i>.<br />Ejemplo: <b>'.$no_avatar.'</b><br /><br />';
			...
			echo'<input type="button" class="login" value="Previsualizar" onclick="load_new_avatar()" /><br /><label id="errorss"></label><label id="sinavatar"><input name="sinavatar" id="sinavatar" value="1" type="checkbox" '; if(empty($context['avatar_url']) || $context['avatar_url']==$no_avatar || $context['avatar_url']=='http://'){echo'checked="checked" ';}  echo'> Sin avatar <span style="font-size:10px;">(avatar default)</span>.</label></center>
							</td>
						</tr>
...
global $tranfer1, $context, $settings, $db_prefix, $options, $scripturl, $modSettings, $txt;
...
echo'<div style="float:left;width: 776px;"><form action="/web/cw-perfilEditar.php" method="post" accept-charset="'.$context['character_set'].'" name="creator" id="creator" enctype="multipart/form-data"><div class="box_780"><div class="box_title" style="width: 774px;"><div class="box_txt box_780-34"><center>'; if($getid){echo'Editar el perfil';}else{echo'Editar mi perfil';}echo'</center></div><div class="box_rss"><img alt="" src="'.$tranfer1.'/blank.gif" style="width:16px;height:16px;" border="0" /></div></div>
...
<option '; if($context['member']['birth_date']['month'] == '1'){echo'selected="selected"';} echo' value="1">enero</option>
<option '; if($context['member']['birth_date']['month'] == '2'){echo'selected="selected"';} echo' value="2">febrero</option>
<option '; if($context['member']['birth_date']['month'] == '3'){echo'selected="selected"';} echo' value="3">marzo</option>
<option '; if($context['member']['birth_date']['month'] == '4'){echo'selected="selected"';} echo' value="4">abril</option>
<option '; if($context['member']['birth_date']['month'] == '5'){echo'selected="selected"';} echo' value="5">mayo</option>
<option '; if($context['member']['birth_date']['month'] == '6'){echo'selected="selected"';} echo' value="6">junio</option>
<option '; if($context['member']['birth_date']['month'] == '7'){echo'selected="selected"';} echo' value="7">julio</option>
<option '; if($context['member']['birth_date']['month'] == '8'){echo'selected="selected"';} echo' value="8">agosto</option>
<option '; if($context['member']['birth_date']['month'] == '9'){echo'selected="selected"';} echo' value="9">septiembre</option>
<option '; if($context['member']['birth_date']['month'] == '10'){echo'selected="selected"';} echo' value="10">octubre</option>
<option '; if($context['member']['birth_date']['month'] == '11'){echo'selected="selected"';} echo' value="11">noviembre</option>
<option '; if($context['member']['birth_date']['month'] == '12'){echo'selected="selected"';} echo' value="12">diciembre</option>
</select>
<select tabindex="', $context['tabindex']++, '" name="bday3" id="bday3" autocomplete="off">
<option value="">A&ntilde;o:</option>
<option '; if($context['member']['birth_date']['year'] == '2003'){echo'selected="selected"';} echo' value="2003">2003</option>
<option '; if($context['member']['birth_date']['year'] == '2002'){echo'selected="selected"';} echo' value="2002">2002</option>
<option '; if($context['member']['birth_date']['year'] == '2001'){echo'selected="selected"';} echo' value="2001">2001</option>
<option '; if($context['member']['birth_date']['year'] == '2000'){echo'selected="selected"';} echo' value="2000">2000</option>
<option '; if($context['member']['birth_date']['year'] == '1999'){echo'selected="selected"';} echo' value="1999">1999</option>
<option '; if($context['member']['birth_date']['year'] == '1998'){echo'selected="selected"';} echo' value="1998">1998</option>
<option '; if($context['member']['birth_date']['year'] == '1997'){echo'selected="selected"';} echo' value="1997">1997</option>
<option '; if($context['member']['birth_date']['year'] == '1996'){echo'selected="selected"';} echo' value="1996">1996</option>
<option '; if($context['member']['birth_date']['year'] == '1995'){echo'selected="selected"';} echo' value="1995">1995</option>
<option '; if($context['member']['birth_date']['year'] == '1994'){echo'selected="selected"';} echo' value="1994">1994</option>

<option '; if($context['member']['birth_date']['year'] == '1993'){echo'selected="selected"';} echo' value="1993">1993</option>
<option '; if($context['member']['birth_date']['year'] == '1992'){echo'selected="selected"';} echo' value="1992">1992</option>
<option '; if($context['member']['birth_date']['year'] == '1991'){echo'selected="selected"';} echo' value="1991">1991</option>
<option '; if($context['member']['birth_date']['year'] == '1990'){echo'selected="selected"';} echo' value="1990">1990</option>
<option '; if($context['member']['birth_date']['year'] == '1989'){echo'selected="selected"';} echo' value="1989">1989</option>
<option '; if($context['member']['birth_date']['year'] == '1988'){echo'selected="selected"';} echo' value="1988">1988</option>
<option '; if($context['member']['birth_date']['year'] == '1987'){echo'selected="selected"';} echo' value="1987">1987</option>
<option '; if($context['member']['birth_date']['year'] == '1986'){echo'selected="selected"';} echo' value="1986">1986</option>
<option '; if($context['member']['birth_date']['year'] == '1985'){echo'selected="selected"';} echo' value="1985">1985</option>

<option '; if($context['member']['birth_date']['year'] == '1984'){echo'selected="selected"';} echo' value="1984">1984</option>
<option '; if($context['member']['birth_date']['year'] == '1983'){echo'selected="selected"';} echo' value="1983">1983</option>
<option '; if($context['member']['birth_date']['year'] == '1982'){echo'selected="selected"';} echo' value="1982">1982</option>
<option '; if($context['member']['birth_date']['year'] == '1981'){echo'selected="selected"';} echo' value="1981">1981</option>
<option '; if($context['member']['birth_date']['year'] == '1980'){echo'selected="selected"';} echo' value="1980">1980</option>
<option '; if($context['member']['birth_date']['year'] == '1979'){echo'selected="selected"';} echo' value="1979">1979</option>
<option '; if($context['member']['birth_date']['year'] == '1978'){echo'selected="selected"';} echo' value="1978">1978</option>
<option '; if($context['member']['birth_date']['year'] == '1977'){echo'selected="selected"';} echo' value="1977">1977</option>
<option '; if($context['member']['birth_date']['year'] == '1976'){echo'selected="selected"';} echo' value="1976">1976</option>

<option '; if($context['member']['birth_date']['year'] == '1975'){echo'selected="selected"';} echo' value="1975">1975</option>
<option '; if($context['member']['birth_date']['year'] == '1974'){echo'selected="selected"';} echo' value="1974">1974</option>
<option '; if($context['member']['birth_date']['year'] == '1973'){echo'selected="selected"';} echo' value="1973">1973</option>
<option '; if($context['member']['birth_date']['year'] == '1972'){echo'selected="selected"';} echo' value="1972">1972</option>
<option '; if($context['member']['birth_date']['year'] == '1971'){echo'selected="selected"';} echo' value="1971">1971</option>
<option '; if($context['member']['birth_date']['year'] == '1970'){echo'selected="selected"';} echo' value="1970">1970</option>
<option '; if($context['member']['birth_date']['year'] == '1969'){echo'selected="selected"';} echo' value="1969">1969</option>
<option '; if($context['member']['birth_date']['year'] == '1968'){echo'selected="selected"';} echo' value="1968">1968</option>
<option '; if($context['member']['birth_date']['year'] == '1967'){echo'selected="selected"';} echo' value="1967">1967</option>

<option '; if($context['member']['birth_date']['year'] == '1966'){echo'selected="selected"';} echo' value="1966">1966</option>
<option '; if($context['member']['birth_date']['year'] == '1965'){echo'selected="selected"';} echo' value="1965">1965</option>
<option '; if($context['member']['birth_date']['year'] == '1964'){echo'selected="selected"';} echo' value="1964">1964</option>
<option '; if($context['member']['birth_date']['year'] == '1963'){echo'selected="selected"';} echo' value="1963">1963</option>
<option '; if($context['member']['birth_date']['year'] == '1962'){echo'selected="selected"';} echo' value="1962">1962</option>
<option '; if($context['member']['birth_date']['year'] == '1961'){echo'selected="selected"';} echo' value="1961">1961</option>
<option '; if($context['member']['birth_date']['year'] == '1960'){echo'selected="selected"';} echo' value="1960">1960</option>
<option '; if($context['member']['birth_date']['year'] == '1959'){echo'selected="selected"';} echo' value="1959">1959</option>
<option '; if($context['member']['birth_date']['year'] == '1958'){echo'selected="selected"';} echo' value="1958">1958</option>

<option '; if($context['member']['birth_date']['year'] == '1957'){echo'selected="selected"';} echo' value="1957">1957</option>
<option '; if($context['member']['birth_date']['year'] == '1956'){echo'selected="selected"';} echo' value="1956">1956</option>
<option '; if($context['member']['birth_date']['year'] == '1955'){echo'selected="selected"';} echo' value="1955">1955</option>
<option '; if($context['member']['birth_date']['year'] == '1954'){echo'selected="selected"';} echo' value="1954">1954</option>
<option '; if($context['member']['birth_date']['year'] == '1953'){echo'selected="selected"';} echo' value="1953">1953</option>
<option '; if($context['member']['birth_date']['year'] == '1952'){echo'selected="selected"';} echo' value="1952">1952</option>
<option '; if($context['member']['birth_date']['year'] == '1951'){echo'selected="selected"';} echo' value="1951">1951</option>
<option '; if($context['member']['birth_date']['year'] == '1950'){echo'selected="selected"';} echo' value="1950">1950</option>
<option '; if($context['member']['birth_date']['year'] == '1949'){echo'selected="selected"';} echo' value="1949">1949</option>

<option '; if($context['member']['birth_date']['year'] == '1948'){echo'selected="selected"';} echo' value="1948">1948</option>
<option '; if($context['member']['birth_date']['year'] == '1901'){echo'selected="selected"';} echo' value="1947">1947</option>
<option '; if($context['member']['birth_date']['year'] == '1946'){echo'selected="selected"';} echo' value="1946">1946</option>
<option '; if($context['member']['birth_date']['year'] == '1945'){echo'selected="selected"';} echo' value="1945">1945</option>
<option '; if($context['member']['birth_date']['year'] == '1944'){echo'selected="selectted"';} echo' value="1944">1944</option>
<option '; if($context['member']['birth_date']['year'] == '1943'){echo'selected="selected"';} echo' value="1943">1943</option>
<option '; if($context['member']['birth_date']['year'] == '1942'){echo'selected="selected"';} echo' value="1942">1942</option>
<option '; if($context['member']['birth_date']['year'] == '1941'){echo'selected="selected"';} echo' value="1941">1941</option>
<option '; if($context['member']['birth_date']['year'] == '1940'){echo'selected="selected"';} echo' value="1940">1940</option>

<option '; if($context['member']['birth_date']['year'] == '1939'){echo'selected="selected"';} echo' value="1939">1939</option>
<option '; if($context['member']['birth_date']['year'] == '1938'){echo'selected="selected"';} echo' value="1938">1938</option>
<option '; if($context['member']['birth_date']['year'] == '1937'){echo'selected="selected"';} echo' value="1937">1937</option>
<option '; if($context['member']['birth_date']['year'] == '1936'){echo'selected="selected"';} echo' value="1936">1936</option>
<option '; if($context['member']['birth_date']['year'] == '1935'){echo'selected="selected"';} echo' value="1935">1935</option>
<option '; if($context['member']['birth_date']['year'] == '1934'){echo'selected="selected"';} echo' value="1934">1934</option>
<option '; if($context['member']['birth_date']['year'] == '1933'){echo'selected="selected"';} echo' value="1933">1933</option>
<option '; if($context['member']['birth_date']['year'] == '1932'){echo'selected="selected"';} echo' value="1932">1932</option>
<option '; if($context['member']['birth_date']['year'] == '1931'){echo'selected="selected"';} echo' value="1931">1931</option>

<option '; if($context['member']['birth_date']['year'] == '1930'){echo'selected="selected"';} echo' value="1930">1930</option>
<option '; if($context['member']['birth_date']['year'] == '1929'){echo'selected="selected"';} echo' value="1929">1929</option>
<option '; if($context['member']['birth_date']['year'] == '1928'){echo'selected="selected"';} echo' value="1928">1928</option>
<option '; if($context['member']['birth_date']['year'] == '1927'){echo'selected="selected"';} echo' value="1927">1927</option>
<option '; if($context['member']['birth_date']['year'] == '1926'){echo'selected="selected"';} echo' value="1926">1926</option>
<option '; if($context['member']['birth_date']['year'] == '1925'){echo'selected="selected"';} echo' value="1925">1925</option>
<option '; if($context['member']['birth_date']['year'] == '1924'){echo'selected="selected"';} echo' value="1924">1924</option>
<option '; if($context['member']['birth_date']['year'] == '1923'){echo'selected="selected"';} echo' value="1923">1923</option>
<option '; if($context['member']['birth_date']['year'] == '1922'){echo'selected="selected"';} echo' value="1922">1922</option>

<option '; if($context['member']['birth_date']['year'] == '1921'){echo'selected="selected"';} echo' value="1921">1921</option>
<option '; if($context['member']['birth_date']['year'] == '1920'){echo'selected="selected"';} echo' value="1920">1920</option>
<option '; if($context['member']['birth_date']['year'] == '1919'){echo'selected="selected"';} echo' value="1919">1919</option>
<option '; if($context['member']['birth_date']['year'] == '1918'){echo'selected="selected"';} echo' value="1918">1918</option>
<option '; if($context['member']['birth_date']['year'] == '1917'){echo'selected="selected"';} echo' value="1917">1917</option>
<option '; if($context['member']['birth_date']['year'] == '1916'){echo'selected="selected"';} echo' value="1916">1916</option>
<option '; if($context['member']['birth_date']['year'] == '1915'){echo'selected="selected"';} echo' value="1915">1915</option>
<option '; if($context['member']['birth_date']['year'] == '1914'){echo'selected="selected"';} echo' value="1914">1914</option>
<option '; if($context['member']['birth_date']['year'] == '1913'){echo'selected="selected"';} echo' value="1913">1913</option>

<option '; if($context['member']['birth_date']['year'] == '1912'){echo'selected="selected"';} echo' value="1912">1912</option>
<option '; if($context['member']['birth_date']['year'] == '1911'){echo'selected="selected"';} echo' value="1911">1911</option>
<option '; if($context['member']['birth_date']['year'] == '1910'){echo'selected="selected"';} echo' value="1910">1910</option>
<option '; if($context['member']['birth_date']['year'] == '1909'){echo'selected="selected"';} echo' value="1909">1909</option>
<option '; if($context['member']['birth_date']['year'] == '1908'){echo'selected="selected"';} echo' value="1908">1908</option>
<option '; if($context['member']['birth_date']['year'] == '1907'){echo'selected="selected"';} echo' value="1907">1907</option>
<option '; if($context['member']['birth_date']['year'] == '1906'){echo'selected="selected"';} echo' value="1906">1906</option>
<option '; if($context['member']['birth_date']['year'] == '1905'){echo'selected="selected"';} echo' value="1905">1905</option>
<option '; if($context['member']['birth_date']['year'] == '1904'){echo'selected="selected"';} echo' value="1904">1904</option>

<option '; if($context['member']['birth_date']['year'] == '1903'){echo'selected="selected"';} echo' value="1903">1903</option>
<option '; if($context['member']['birth_date']['year'] == '1902'){echo'selected="selected"';} echo' value="1902">1902</option>
...
<option '; if($context['member']['birth_date']['year'] == '1900'){echo'selected="selected"';} echo' value="1900">1900</option>
						...
						<option '; if($context['member']['title'] == 'co'){echo'selected="selected"';} echo' value="co">Colombia</option>
						...
						<option '; if($context['member']['title'] == 'as'){echo'selected="selected"';} echo' value="es">Espa&ntilde;a</option>
...
echo'<tr>						<td width="40%"><b class="size11">Mostrar apariencia a:</b></td>
								...
								<option value="0" '; if(!$quien){echo'selected="selected"';} echo'>A todos</option>
								<option value="1" '; if($quien=='1'){echo'selected="selected"';} echo'>Nadie</option>
								<option value="2" '; if($quien=='2'){echo'selected="selected"';} echo'>Amigos</option>
								<option value="3" '; if($quien=='3'){echo'selected="selected"';} echo'>Registrados</option>
								</select>
								</td>
							</tr>
...
echo'<option value="0" '; if(!$recibirmail){echo'selected="selected"';} echo'>No</option>
<option value="1" '; if($recibirmail=='1'){echo'selected="selected"';} echo'>Si</option>
								</select>
								</td>
							</tr>
...
function template_profile_save(){global $tranfer1, $context, $settings, $options, $txt;
...
<td><input type="password" onfocus="foco(this);" onblur="no_foco(this);" name="oldpasswrd" size="20" onfocus="javascript:select();" style="margin-right: 4ex;" />';}else{echo'<td align="center" colspan="2">';
echo'<br /><br />
...
if($_GET['u']){echo'<input type="hidden" name="llegaravatar" value="'.$_GET['u'].'"/>';}
