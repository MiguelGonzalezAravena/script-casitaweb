...
$message=str_replace("\n","<br />",trim($message));
if(!$message){fatal_error('Falt el mensaje.');}
...
if($id_de==$para){die('0: No te puedes autoenviar mensaje.');}

if(empty($sis)){$sistema='0';}else{$sistema='1';}
if(empty($sistema)){$eliminadoss='0';}else{$eliminadoss='1';}

$htmlmessage=trim($func['htmlspecialchars']($mensaje, ENT_QUOTES));
$titulo=trim($func['htmlspecialchars']($titulo));

if(empty($titulo)){die('0: El mensaje debe tener un asunto.');}
if(empty($htmlmessage)){die('0: Debes enviar un mensaje.');}

if(strlen($titulo)>=150){die('0: El asunto es muy largo.');}
if(strlen($htmlmessage)>=5534){die('0:El mensaje es muy largo.');}

db_query("INSERT INTO {$db_prefix}mensaje_personal (name_de, id_de, id_para,eliminado_de, fecha, titulo, mensaje,sistema) VALUES ('$name_de', '$id_de', '$para', '$eliminadoss', ".time().", '$titulo','$htmlmessage','$sistema')", __FILE__, __LINE__);
db_query("UPDATE {$db_prefix}members SET topics=topics+1 WHERE ID_MEMBER='$para' LIMIT 1", __FILE__, __LINE__);
