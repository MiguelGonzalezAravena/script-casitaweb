...
function RegCenter(){global $modSettings, $context, $txt, $db_prefix, $scripturl;
    ...
    if(!$context['user']['is_admin']){fatal_error('No podes estar aca');}
	...
	$context['admin_tabs'] = array(
		...
		'tabs' => array(
			'register' => array(
				...
				'href' => '/moderacion/registro/register/',
	...
	if (!empty($_POST['regSubmit']))
	{
		...
		if (!empty($memberID))
		{
			$context['new_member'] = array(
				'id' => $memberID,
				'name' => $_POST['user'],
				'href' => '/perfil/'.$_POST['user'],
				'link' => '<a href="/perfil/'.$_POST['user'].'">'.$_POST['user'].'</a>',
			);
...
function EditAgreement(){global $txt, $boarddir, $context, $modSettings,$db_prefix;
if (isset($_POST['agreement'])){
$_POST['agreement']=trim($_POST['agreement']);
if(empty($_POST['agreement'])){$texto='';}else{$texto=$_POST['agreement'];}
updateSettings(array('requireAgreement' => !empty($_POST['requireAgreement'])));

db_query("UPDATE {$db_prefix}settings SET value='$texto' WHERE variable='terminos' LIMIT 1", __FILE__, __LINE__);
...
function AdminSettings()
{
	...
	if (isset($_POST['save']))
	{
		...
		redirectexit('http://casitaweb.net/moderacion/registro/settings/');
	}
