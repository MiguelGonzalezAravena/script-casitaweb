...
$context['sticky']=array(); while ($row=mysql_fetch_assoc($rsdd)){ $context['sticky'][]=array(
		'id' => $row['ID_TOPIC'],
		'titulo' => censorText($row['subject']),
		'description' => $row['description'],
		'ocultar' => $row['hiddenOption'],
...
while ($row= mysql_fetch_assoc($ultimasimg)){
    $context['ultimas_img'][] = array(
			'id' => $row['ID_PICTURE'],
			'titulo' => $row['title']);}
mysql_free_result($ultimasimg);
...
while ($row_members = mysql_fetch_assoc($members_result)){$context['top_starters'][] = array(
