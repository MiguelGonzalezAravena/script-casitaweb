...
if($pags33<1){$per='1';}else{$per=$pags33;}
if(isset($per)){$RegistrosAEmpezar=($per-1)*$RegistrosAMostrar;
$context['PagAct']=$per;}else{$RegistrosAEmpezar=0;$context['PagAct']=1;}
$context['PagAnt']=$context['PagAct']-1;
$context['PagSig']=$context['PagAct']+1;
$context['PagUlt']=$NroRegistros/$RegistrosAMostrar;
$Res=$NroRegistros%$RegistrosAMostrar;
if($Res>0) $context['PagUlt']=floor($PagUlt)+1;
//�ltimos posts
$rs=db_query("SELECT m.subject, b.description, m.hiddenOption, b.ID_BOARD, m.ID_BOARD, m.ID_TOPIC, b.name
FROM ({$db_prefix}messages AS m)
INNER JOIN {$db_prefix}boards as b ON m.ID_BOARD=b.ID_BOARD AND m.eliminado=0". (empty($context['catccdd']) ? '' : " AND b.description='{$context['catccdd']}'")."
...
INNER JOIN {$db_prefix}boards AS b ON b.ID_BOARD=p.ID_BOARD AND p.eliminado=0 AND p.sticky=1". (empty($context['catccdd']) ? '' : " AND b.description='{$context['catccdd']}'")."
ORDER BY p.ID_TOPIC DESC 
LIMIT 7",__FILE__, __LINE__);
$context['sticky']=array(); while ($row=mysqli_fetch_assoc($rsdd)){ $context['sticky'][]=array(
    'id' => $row['ID_TOPIC'],
    'titulo' => censorText($row['subject']),
    'description' => $row['description'],
    'ocultar' => $row['hiddenOption'],
...
while ($row= mysqli_fetch_assoc($ultimasimg)){
    $context['ultimas_img'][] = array(
      'id' => $row['ID_PICTURE'],
      'titulo' => $row['title']);}
mysqli_free_result($ultimasimg);
...
while ($row_members = mysqli_fetch_assoc($request)){
    $context['top_posters_week'][] = array(
      ...
      'href' => '/perfil/' . $row_members['memberName'],
      'link' => '<a href="/perfil/' . $row_members['memberName'] . '">' . $row_members['realName'] . '</a>'
    );
...
$request = db_query("
    ...
    WHERE me.ID_TOPIC = m.id_post AND me.ID_BOARD=b.ID_BOARD AND
        m.fecha > " . $tiempo . " AND m.id_post != 0 AND me.eliminado=0
...
while ($row_members = mysqli_fetch_assoc($members_result)){$context['top_starters'][] = array(
