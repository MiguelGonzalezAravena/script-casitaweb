...
function ModifyMembergroups(){global $context, $txt, $scripturl;
	$subActions = array(
	...
	if (allowedTo('admin_forum'))
		$context['admin_tabs']['tabs']['settings'] = array(
			'title' => $txt['settings'],
			'description' => $txt['membergroups_description'],
			'href' => $scripturl . '?cw1=membergroups;sa=settings',
...
{
	global $tranfer1,$db_prefix, $txt, $scripturl, $context, $settings;
	...
	while ($row = mysql_fetch_assoc($query))
	{
		...
		$context['groups'][$row['minPosts'] == -1 ? 'regular' : 'post'][$row['ID_GROUP']] = array(
			...
			'href' => $scripturl . '?cw1=membergroups;sa=members;group=' . $row['ID_GROUP'],
...
function AddMembergroup()
{
	...
	Header("Location: /index.php?cw1=membergroups;sa=edit;group={$ID_GROUP}");exit();die();
	}
...
function DeleteMembergroup(){global $sourcedir;
checkSession('get');
require($sourcedir . '/Subs-Members.php');
deleteMembergroups((int) $_REQUEST['group']);
Header("Location: /index.php?cw1=membergroups");exit();die();}

function EditMembergroup(){global $db_prefix, $context, $txt, $sourcedir;
...
function MembergroupMembers()
{
	...
	$context['page_index'] = constructPageIndex($scripturl . '?cw1=membergroups;sa=members;group=' . $_REQUEST['group'] . ';sort=' . $context['sort_by'] . (isset($_REQUEST['desc']) ? ';desc' : ''), $_REQUEST['start'], $context['total_members'], $modSettings['defaultMaxMembers']);
