...
function ModifyProfile($post_errors = array()) {
  ...
  if (!isset($_REQUEST['sa']) || !isset($sa_allowed[$_REQUEST['sa']])) {
    if ((allowedTo('profile_view_own') && $context['user']['is_owner']) || allowedTo('profile_view_any')) {
      ...
      $_REQUEST['sa'] = 'cuenta';
    } else if ((allowedTo('profile_extra_own') && $context['user']['is_owner']) || allowedTo('profile_extra_any')) {
      $_REQUEST['sa'] = 'perfil';
    } else {
  ...
  if ($changeIdentity) {
    ...
    if (isset($_POST['emailAddress']) && strtolower($_POST['emailAddress']) != strtolower($old_profile['emailAddress'])) {
      ...
      if (preg_match('~^[0-9A-Za-z=_+\-/][0-9A-Za-z=_\'+\-/\.]*@[\w\-]+(\.[\w\-]+)*(\.[\w]{2,6})$~', stripslashes($_POST['emailAddress'])) == 0) {
        fatal_error('Car&aacute;cteres no v&aacute;lidos en el e-mail.');
      }
...
function summary($memID) {
  ...
  if ($ID_MEMBER = $memID) {
    $request = db_query("
      ...
      INNER JOIN {$db_prefix}members AS m ON p.ID_MEMBER='$memID'
      AND p.ID_MEMBER = m.ID_MEMBER", __FILE__, __LINE__);

    $context['count'] = mysqli_num_rows($request);
