...
function reloadSettings(){global $modSettings,$mysql_set_mode, $context, $db_prefix, $boarddir, $func, $txt, $db_character_set;
...
function loadUserSettings(){global $modSettings, $user_settings;
	...
	if ($ID_MEMBER != 0)
	{
		...
		{
if(!empty($_GET['accioncw241'])){$cw2=$_GET['accioncw241'];}else{$cw2='vacio';}		  
 if($cw2 == 'monitorUser'){db_query("UPDATE {$db_prefix}members SET notificacionMonitor=0 WHERE ID_MEMBER='{$ID_MEMBER}'", __FILE__, __LINE__);}
...
if ($ID_MEMBER != 0){if (empty($_SESSION['ID_MSG_LAST_VISIT']))
$_SESSION['ID_MSG_LAST_VISIT'] = $user_settings['ID_MSG_LAST_VISIT'];
$username=$user_settings['memberName'];
if (empty($user_settings['additionalGroups']))
$user_info=array('groups' => array($user_settings['ID_GROUP'], $user_settings['ID_POST_GROUP'])); else
$user_info=array('groups' => array_merge(array($user_settings['ID_GROUP'], $user_settings['ID_POST_GROUP']),
explode(',', $user_settings['additionalGroups'])));}else{
$username='';
...
function loadTheme($ID_THEME = 0, $initialize = true){
	...
	if (isset($detected_url) && $detected_url != $boardurl)
	{
		if (!empty($modSettings['forum_alias_urls']))
		{
			...
			{	if ($detected_url == trim($alias) || strtr($detected_url, array('http://' => '', 'https://' => '')) == trim($alias))  $do_fix = true;
			}
		}
		...
		if (strtr($detected_url, array('https://' => 'http://')) == $boardurl)$do_fix = true;
...
$user_info['smiley_set'] = (!in_array($user_info['smiley_set'], explode(',', $modSettings['smiley_sets_known'])) && $user_info['smiley_set'] != 'none') || empty($modSettings['smiley_sets_enable']) ? (!empty($settings['smiley_sets_default']) ? $settings['smiley_sets_default'] : $modSettings['smiley_sets_default']) : $user_info['smiley_set'];
	...
	if ((isset($_GET['accioncw241']) && in_array($_GET['accioncw241'], array('login', 'login2', 'register'))) || !$context['user']['is_guest'])$context['browser']['possibly_robot'] = false;
	$txt = array();
...
LIMIT 1", __FILE__, __LINE__);
while($row=mysql_fetch_assoc($request)){$cp=$row['CantidadDePuntos'];}
...
if($idtop > 0){    
$request=db_query("
SELECT p.ID_BOARD,p.ID_TOPIC,p.puntos,p.subject,p.body,p.ID_MEMBER,p.hiddenOption,p.anuncio,b.ID_BOARD,b.name,b.description,p.posterTime,p.smileysEnabled,p.visitas,SUBSTRING(p.body, 1,300) AS ACsgg,p.posterName,p.eliminado
FROM ({$db_prefix}messages AS p)
INNER JOIN {$db_prefix}boards as b ON p.ID_BOARD=b.ID_BOARD AND p.ID_TOPIC='$idtop'
LIMIT 1", __FILE__, __LINE__);
while($row=mysql_fetch_assoc($request)){
...
function &censorText(&$text){global $modSettings, $options, $settings, $txt;
...
$text = preg_replace($censor_vulgar, $censor_proper, $text);return $text;}
...
function template_include($filename, $once = false){
	...
	if ($once && in_array($filename, $templates))return;
	else
		$templates[] = $filename;
...
if (isset($context['character_set']))echo'<meta http-equiv="Content-Type" content="text/html; charset=', $context['character_set'], '" />'; echo'</head><body>'.$mtitle.'</body></html>';
die;exit;}
}
...
function sessionGC($max_lifetime){global $db_prefix, $modSettings;
...
function get_memcached_server($level = 3){global $modSettings, $memcached, $db_persist;
	$servers = explode(',', $modSettings['cache_memcached']);
	$server = explode(':', trim($servers[array_rand($servers)]));
	$level = min(count($servers), $level);
	if (empty($db_persist))$memcached = @fsockopen($server[0], empty($server[1]) ? 11211 : $server[1], $err, $err, 0.15);
	else $memcached = @pfsockopen($server[0], empty($server[1]) ? 11211 : $server[1], $err, $err, 0.15);
	if (!$memcached && $level > 0)get_memcached_server($level - 1);
	elseif ($memcached){@socket_set_timeout($memcached, 1);@set_file_buffer($memcached, 0);}}
