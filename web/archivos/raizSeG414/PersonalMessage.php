...
function MessageMain() {
  ...
  if ($accion == 'redactar') {
    $context['page_title'] = 'Enviar mensaje';
  } else if ($accion == 'enviar') {
    $context['page_title'] = $txt[18];
  } else if ($accion == 'enviados') {
    $context['page_title'] = 'Mensajes enviados';
  } else if ($accion == 'leer') {
    $context['page_title'] = $txt[18];
  } else if ($accion == 'leere') {
    $context['page_title'] = $txt[18];
  } else if ($accion == 'eliminar') {
    $context['page_title'] = $txt[18];
  } else if ($accion == 'recibidos') {
    $context['page_title'] = 'Mensajes recibidos';
  } else {
    $context['page_title'] = $txt[18];
  }
}
  ...
  if ($context['user']['is_admin']) {
    if (!empty($id)) {
      db_query("
        ...
        WHERE id_de = $id", __FILE__, __LINE__);

      db_query("
        DELETE FROM {$db_prefix}mensaje_personal
        WHERE id_para = $id", __FILE__, __LINE__);
    }
  }
