...
function ModifyPermissions(){global $txt, $scripturl, $context;
	...
	if (allowedTo('manage_permissions'))
	{
		$context['admin_tabs']['tabs']['index'] = array(
			...
			'href' => $scripturl . '?cw1=permissions',
		...
		$context['admin_tabs']['tabs']['board_permissions'] = array(
			...
			'href' => $scripturl . '?cw1=permissions;sa=board',
	...
	if (allowedTo('admin_forum'))
		$context['admin_tabs']['tabs']['settings'] = array(
			...
			'href' => $scripturl . '?cw1=permissions;sa=settings',
...
function PermissionIndex()
{
	...
	$context['groups'] = array(
		...
		0 => array(
			...
			'allow_modify' => true,
			'can_search' => true,
			'href' => $scripturl . '?cw1=viewmembers;sa=query;params=' . base64_encode('ID_GROUP = 0'),
			'is_post_group' => false,
			'color' => '',
			'stars' => '',
			'num_permissions' => array(
	...
	while ($row = mysqli_fetch_assoc($query))
	{
		...
		$context['groups'][$row['ID_GROUP']] = array(
			...
			'href' => $scripturl . '?cw1=viewmembers;sa=query;params=' . base64_encode($row['minPosts'] == -1 ? "ID_GROUP = $row[ID_GROUP] OR FIND_IN_SET($row[ID_GROUP], additionalGroups)" : "ID_POST_GROUP = $row[ID_GROUP]"),
...
function SetQuickGroups()
{
	...
	if (empty($_POST['group']))
		redirectexit('cw1=permissions;boardid=' . $_REQUEST['boardid']);
...
function SwitchBoard()
{
	...
	if ($_GET['to'] == 'local')
		...
		redirectexit('cw1=permissions;sa=board');
}
...
function ModifyMembergroup2()
{
	...
	redirectexit('cw1=permissions;boardid=' . $_GET['boardid']);
}
