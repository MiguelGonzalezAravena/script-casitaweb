...
function ModifyPermissions()
{
  ...
  if (allowedTo('manage_permissions')) {
    ...
    $context['admin_tabs']['tabs']['board_permissions'] = array(
      ...
      'href' => $scripturl . '?' . $urlSep . '=permissions;sa=board',
  ...
  if (allowedTo('admin_forum')) {
    $context['admin_tabs']['tabs']['settings'] = array(
      ...
      'href' => $scripturl . '?' . $urlSep . '=permissions;sa=settings',
...
function PermissionIndex()
{
  ...
  $context['groups'] = array(
    ...
    0 => array(
      ...
      'allow_modify' => true,
      'can_search' => true,
      'href' => $scripturl . '?' . $urlSep . '=viewmembers;sa=query;params=' . base64_encode('ID_GROUP = 0'),
      'is_post_group' => false,
      'color' => '',
      'stars' => '',
      'num_permissions' => array(
  ...
  while ($row = mysqli_fetch_assoc($query)) {
    ...
    $context['groups'][$row['ID_GROUP']] = array(
      ...
      'href' => $scripturl . '?' . $urlSep . '=viewmembers;sa=query;params=' . base64_encode($row['minPosts'] == -1 ? "ID_GROUP = $row[ID_GROUP] OR FIND_IN_SET($row[ID_GROUP], additionalGroups)" : "ID_POST_GROUP = $row[ID_GROUP]"),
      'is_post_group' => $row['minPosts'] != -1,
      'color' => empty($row['onlineColor']) ? '' : $row['onlineColor'],
      'stars' => !empty($row['stars'][0]) && !empty($row['stars'][1]) ? str_repeat('<img src="' . $tranfer1 . '/' . $row['stars'][1] . '" alt="*" border="0" />', $row['stars'][0]) : '',
      'num_permissions' => array(
...
function SwitchBoard()
{
  ...
  if (empty($modSettings['permission_enable_by_board']))
    redirectexit($urlSep . '=permissions');
