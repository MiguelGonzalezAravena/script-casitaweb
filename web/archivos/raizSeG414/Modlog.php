...
function ViewModlog(){global $db_prefix, $txt, $modSettings, $context, $scripturl;
loadTemplate('Modlog');
$context['page_title'] = 'Historial de moderaci&oacute;n';
if(!$context['user']['id']){is_not_guest();}
	$descriptions = array(
	...
	$result = db_query("
		SELECT
			lm.ID_ACTION, lm.ID_MEMBER,  lm.action, lm.extra,
			mem.realName
	...
	while ($row = mysqli_fetch_assoc($result))
	{
		...
		$context['entries'][$row['ID_ACTION']] = array(
			'id' => $row['ID_ACTION'],
			'que' => $sis,
			'moderator' => array('link' => '<a href="/perfil/' . $row['realName'] . '">' . $row['realName'] . '</a>'),
			'extra' => $row['extra'],
