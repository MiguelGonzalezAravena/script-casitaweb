...
LIMIT 10", __FILE__, __LINE__);
  ...
  while ($row_topic_views = mysqli_fetch_assoc($topic_view_result)) {
    ...
    $context['top_topics_views'][] = array(
      ...
      'board' => array(
        ...
        'href' => $boardurl . '/categoria/' . $row_topic_views['ID_BOARD'] . '',
        'link' => '<a href="' . $boardurl . '/categoria/' . $row_topic_views['ID_BOARD'] . '">' . $row_topic_views['name'] . '</a>'
      ),
      'subject' => $row_topic_views['subject'],
      'num_views' => $row_topic_views['visitas'],
      'href' => $boardurl . '/post/' . $row_topic_views['ID_TOPIC'] . '/' . urls($row_topic_views['description']) . '/' . urls($row_topic_views['subject']) . '.html',
      'link' => '<a href="' . $boardurl . '/post/' . $row_topic_views['ID_TOPIC'] . '/' . urls($row_topic_views['description']) . '/' . urls($row_topic_views['subject']) . '.html">' . $row_topic_views['subject'] . '</a>'
    );
  ...
  $request = db_query("
    SELECT m.subject, m.ID_TOPIC, m.puntos, b.description
    FROM ({$db_prefix}messages AS m,{$db_prefix}boards AS b)
  ...
  while ($row_months = mysqli_fetch_assoc($months_result)) {
    ...
    $context['monthly'][$ID_MONTH] = array(
      ...
      'href' => $boardurl . '/tops/' . ($expanded ? 'collapse' : 'expand') . '/' . $ID_MONTH . '#' . $ID_MONTH,
      'link' => '<a href="' . $boardurl . '/tops/' . ($expanded ? 'collapse' : 'expand') . '/' . $ID_MONTH . '#' . $ID_MONTH . '">' . $txt['months'][$row_months['stats_month']] . ' ' . $row_months['stats_year'] . '</a>',
