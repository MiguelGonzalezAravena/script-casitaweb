...
function db_affected_rows(){global $db_connection;return mysql_affected_rows($db_connection);}
function db_insert_id(){global $db_connection;return mysql_insert_id($db_connection);}
...
function updateSettings($changeArray, $update = false){global $db_prefix, $modSettings;
if (empty($changeArray) || !is_array($changeArray))return;
if ($update){foreach ($changeArray as $variable => $value){
			db_query("
	...
	{		if (isset($modSettings[$variable]) && $modSettings[$variable] == stripslashes($value))continue;
...
function updatecon($changeArray, $update){global $db_prefix,$context, $modSettings;
...
function forum_time($use_user_offset = true, $timestamp = null){global $user_info, $modSettings;
if ($timestamp === null)$timestamp = time();
elseif ($timestamp == 0)return 0;
...
function doUBBC($message, $enableSmileys = true){return parse_bbc($message, $enableSmileys);}
...
function parsesmileys(&$message){global $modSettings, $db_prefix, $tranfer1, $txt, $user_info, $context;
...
function highlight_php_code($code){global $context;
...
function writeLog($force = false){global $db_prefix, $ID_MEMBER, $user_info, $user_settings, $sc, $modSettings, $settings, $topic, $board,$boardir,$sourcedir;
...
if ($do_delete){
		db_query("
				...
				WHERE logTime < ".$menos." AND session != '$session_id'", __FILE__, __LINE__);
...
if (db_affected_rows() == 0){$_SESSION['log_time'] = 0;}}
else{$_SESSION['log_time'] = 0;}
...
if (WIRELESS){if ($add)$setLocation = $scripturl . '?' . $setLocation;
$char = strpos($setLocation, '?') === false ? '?' : ';';
if (strpos($setLocation, '#') ==! false)
$setLocation = strtr($setLocation, array('#' => $char . WIRELESS_PROTOCOL . '#'));else$setLocation .= $char . WIRELESS_PROTOCOL;}
	elseif ($add)
...
function estadisticastopic($d=''){global $db_prefix, $modSettings;
if(empty($modSettings['trackStats']))return false;
$date = strftime('%Y-%m-%d', forum_time(false));
$lvccct=db_query("
SELECT date
...
ORDER BY date DESC
LIMIT 1", __FILE__, __LINE__);
while($asserr=mysql_fetch_assoc($lvccct)){$topicsss=$asserr['date'];}
...
function template_header(){global $context, $settings;

setupThemeContext();

foreach ($context['template_layers'] as $layer){loadSubTemplate($layer . '_above', true);}
...
function text2words($text, $max_chars = 20, $encrypt = false){	global $func, $context;
...
function spamProtection($error_type){return false;}
function url_image_size($url){return false;}
function determineTopicClass(&$topic_context){return false;}
function template_rawdata(){}
function theme_copyright($get_it = false){}
function template_footer(){global $context;
