...
while($lim2=mysql_fetch_assoc($limit3)){$modifiedTime=$lim2['fecha'];}
...
WHERE realName='$para'
LIMIT 1", __FILE__, __LINE__);
while($data=mysql_fetch_assoc($datosmem)){$id_para=$data['ID_MEMBER'];}

$admitir=mysql_num_rows(db_query("SELECT id_user,quien FROM ({$db_prefix}pm_admitir) WHERE id_user='$id_para' AND quien='{$user_settings['ID_MEMBER']}' LIMIT 1", __FILE__, __LINE__));
