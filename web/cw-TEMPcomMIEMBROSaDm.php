...
if(empty($_GET['c'])){echo'<div class="noesta" style="width:541px;margin-bottom:8px;float:left;">Accion no conocida.-</div>';}
...
if(empty($NroRegistros)){echo'<div class="noesta">No hay miembros.</div>';}else{
 
if(($PagAct>1 || $PagAct<$PagUlt) && $PagAct>1){echo'<div class="panador" onclick="pagComunidad2(\''.$_GET['c'].'\',\''.$PagAnt.'\')">&#171; anterior</div>';}

echo'<div id="miem-com">';

$rs=db_query("SELECT m.realName,m.avatar,c.rango,m.usertitle,m.gender,c.id
...
<div style="float:right;width:400px;">
	<ul>
        ...
        <div><div style="float:left;margin-right:5px;margin-left:5px;"><a href="/web/cw-ComunidadesAprobarmem.php?m='.$row['id'].'" title="Aprobar" class="botN1" style="cursor:pointer;color:#fff;text-shadow: #005400 0px 1px 0px;">Aprobar</a></div><div><a href="/web/cw-ComunidadesDesaprobarmem.php?m='.$row['id'].'" title="No aprobar" class="botN2" style="cursor:pointer;text-shadow: #CC0000 0px 1px 0px;color:#fff;">No aprobar</a></div></div></li>';
        
        echo'</ul></div> <div class="clearfix"></div></div>';}
    
echo'</div>';

if(($PagAct>1 || $PagAct<$PagUlt) &&$PagAct<$PagUlt){echo'<div class="panador" onclick="pagComunidad2(\''.$_GET['c'].'\',\''.$PagSig.'\')"><div class="clearfix"></div>siguiente &#187;</div>';}}

echo'</div></div>';


}else{echo'<div class="noesta" style="width:510px;">Accion no conocida.-</div>';}
?>