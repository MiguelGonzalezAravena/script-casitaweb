...
$ya3=mysql_num_rows(db_query("SELECT m.id_com FROM ({$db_prefix}comunidades_miembros AS m) WHERE m.id_com='$id_com' AND m.id_user='{$user_settings['ID_MEMBER']}' LIMIT 1",__FILE__, __LINE__));
if(!$ya3){echo'0: <span class="error">No sos miembro la comunidad</span>';}else{    
    
$ya=mysql_num_rows(db_query("SELECT v.id_tema FROM ({$db_prefix}comunidades_votosArts AS v) WHERE v.id_tema='$id' AND v.id_user='{$user_settings['ID_MEMBER']}' LIMIT 1",__FILE__, __LINE__));
if($ya){echo'0: <span class="error">Ya puntuastes</span>';}else{
$ya2=mysql_num_rows(db_query("SELECT a.id FROM ({$db_prefix}comunidades_articulos AS a) WHERE a.id='$id' AND a.id_user='{$user_settings['ID_MEMBER']}' LIMIT 1",__FILE__, __LINE__));
