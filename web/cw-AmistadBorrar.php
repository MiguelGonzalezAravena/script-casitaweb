...
$ser=mysqli_num_rows(db_query("SELECT ID_MEMBER FROM {$db_prefix}members WHERE ID_MEMBER='$ids' LIMIT 1",__FILE__, __LINE__));
...
while($data=mysqli_fetch_assoc($datosmem)){$id=$data['id'];}
$yadio=mysqli_num_rows($datosmem) != 0 ? true : false; mysqli_free_result($datosmem);
